...
@register_loss("simple_cntr_issl_loss")
class SimpleCntrISSLLoss(ClassyLoss):
    """
...
class SimpleCntrISSLCriterion(nn.Module):
    """
    ...
    def __init__(self, buffer_params, temperature: float):
        super(SimpleCntrISSLCriterion, self).__init__()
    ...
    def forward(self, embedding: list[torch.Tensor]):
        ...
        # here z_pred should be thought as the representation that should to be dot producted with W
        ...
        embeddings_buffer = self.gather_embeddings(W)

        # Step 2: matrix multiply: 64 x 128 with 4096 x 128 = 64 x 4096 and
        # divide by temperature.
        similarity = torch.exp(torch.mm(z_pred, embeddings_buffer.t()) / T)
